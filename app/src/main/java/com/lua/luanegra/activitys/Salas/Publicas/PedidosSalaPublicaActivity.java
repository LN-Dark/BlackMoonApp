...
import com.lua.luanegra.adapters.PedidosSalaPublicaAdapter;
...
public class PedidosSalaPublicaActivity extends AppCompatActivity {

    private String groupUID;
    private ArrayList<String> listaNotifcationAdmins, listaIdsAdmins, idsUserPedidos, listaHoras, listaDatas, listaIdsPedidos;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pedidos_sala_publica);
    ...
    private void GetAllUsers(){
        ...
        listaIdsPedidos = new ArrayList<>();
        idsUserPedidos = new ArrayList<>();
        ...
        dataRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()){
                    if(dataSnapshot.child("pedidos").exists()){
                        for(DataSnapshot childSnapShotBloqedUser : dataSnapshot.child("pedidos").getChildren()){
                            idsUserPedidos.add(Objects.requireNonNull(childSnapShotBloqedUser.child("idUser").getValue()).toString());
                            listaDatas.add(Objects.requireNonNull(childSnapShotBloqedUser.child("data").getValue()).toString());
                            listaHoras.add(Objects.requireNonNull(childSnapShotBloqedUser.child("hora").getValue()).toString());
                            listaIdsPedidos.add(childSnapShotBloqedUser.getKey());
                        }
                    }
                            ...
                            if(dataSnapshot2.exists()){
                                for(DataSnapshot childSnapShotUsers : dataSnapshot2.getChildren()){
                                    if(!Objects.requireNonNull(childSnapShotUsers.getKey()).equals("chat")){
                                        ...
                                        if(idsUserPedidos.contains(newUser.getUid())){
                                            if(!newUser.getUid().equals(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid())){
                                                newUser.setDataPedido(listaDatas.get(idsUserPedidos.indexOf(newUser.getUid())));
                                                newUser.setHoraPedido(listaHoras.get(idsUserPedidos.indexOf(newUser.getUid())));
                                                newUser.setPedidoID(listaIdsPedidos.get(idsUserPedidos.indexOf(newUser.getUid())));
    ...
    private void InitializeRecyclerView() {
        try {
            EmptyRecyclerView mUserList = findViewById(R.id.pedidos_salaPublica_recycler);
            ...
            mUserListAdapter = new PedidosSalaPublicaAdapter(listaUsersSala);
