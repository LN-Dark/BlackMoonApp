...
public class BloquedUserSalaPrivadaAdapter extends EmptyRecyclerView.Adapter<BloquedUserSalaPrivadaAdapter.BloquedUserViewHolder> {

    private final ArrayList<UserObject> userList;
    private DatabaseReference bloquedUsersRef;

    public BloquedUserSalaPrivadaAdapter(ArrayList<UserObject> userList) {
        this.userList = userList;
    }

    @NonNull
    @Override
    public BloquedUserSalaPrivadaAdapter.BloquedUserViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        return new BloquedUserViewHolder(layoutView);
    }

    private static String FCMServer, SinchKey, SinchSecret;

    @Override
    public void onBindViewHolder(@NonNull final BloquedUserSalaPrivadaAdapter.BloquedUserViewHolder bloquedUserViewHolder, int i) {
        ...
        mAppKeysRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    bloquedUsersRef = FirebaseDatabase.getInstance().getReference().child("salasPrivadas").child(userList.get(bloquedUserViewHolder.getAdapterPosition()).getSalaPrivadaID());
                    ...
                    bloquedUserViewHolder.bloquear.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            if (!bloquedUserViewHolder.bloquear.isChecked()) {
                                ...
                                FirebaseDatabase.getInstance().getReference().child("user").child(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid()).child("salasPrivadas").child(userList.get(bloquedUserViewHolder.getAdapterPosition()).getSalaPrivadaID()).setValue(false);
                                JSONObject notification = new JSONObject();
                                JSONObject notifcationBody = new JSONObject();
                                try {
                                    notifcationBody.put("title", v.getResources().getString(R.string.fostedesbloqueado));
