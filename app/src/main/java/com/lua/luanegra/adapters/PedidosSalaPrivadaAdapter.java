...
public class PedidosSalaPrivadaAdapter extends EmptyRecyclerView.Adapter<PedidosSalaPrivadaAdapter.PedidosViewHolder> {

    private final ArrayList<UserObject> userList;
    private DatabaseReference bloquedUsersRef;

    public PedidosSalaPrivadaAdapter(ArrayList<UserObject> userList) {
        this.userList = userList;
    }

    @NonNull
    @Override
    public PedidosSalaPrivadaAdapter.PedidosViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        return new PedidosViewHolder(layoutView);
    }

    private static String FCMServer, SinchKey, SinchSecret;

    @Override
    public void onBindViewHolder(@NonNull final PedidosSalaPrivadaAdapter.PedidosViewHolder pedidosViewHolder, int i) {
                        ...
                        public void onClick(final View v) {
                            ...
                            builder3.setPositiveButton(pedidosViewHolder.itemView.getResources().getString(R.string.aceitar), new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    FirebaseDatabase.getInstance().getReference().child("salasPrivadas").child(userList.get(pedidosViewHolder.getAdapterPosition()).getSalaPrivadaID()).child("users").child(userList.get(pedidosViewHolder.getAdapterPosition()).getUid()).setValue(true).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                        public void onComplete(@NonNull Task<Void> task) {
                                            FirebaseDatabase.getInstance().getReference().child("salasPrivadas").child(userList.get(pedidosViewHolder.getAdapterPosition()).getSalaPrivadaID()).child("pedidos").child(userList.get(pedidosViewHolder.getAdapterPosition()).getPedidoID()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                                @Override
                                                public void onComplete(@NonNull Task<Void> task) {
