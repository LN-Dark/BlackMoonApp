...
public class PedidoFuncionalidadesAdapter extends EmptyRecyclerView.Adapter<PedidoFuncionalidadesAdapter.FunconalidadesViewHolder> {
    private final ArrayList<LogObject> funcionalidadesList;

    public PedidoFuncionalidadesAdapter(ArrayList<LogObject> news) {
        this.funcionalidadesList =  news;
    }

    @NonNull
    @Override
    public PedidoFuncionalidadesAdapter.FunconalidadesViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        return new PedidoFuncionalidadesAdapter.FunconalidadesViewHolder(layoutView);
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull final PedidoFuncionalidadesAdapter.FunconalidadesViewHolder FunconalidadesViewHolder, int i) {
        try{
            loadSuperAdmins();
            FunconalidadesViewHolder.mLogText.setText(funcionalidadesList.get(i).getLogText());
            FunconalidadesViewHolder.mLoguserID.setText(funcionalidadesList.get(FunconalidadesViewHolder.getAdapterPosition()).getLogCreator());
            FunconalidadesViewHolder.mLayout.setOnLongClickListener(new View.OnLongClickListener() {
                @Override
                public boolean onLongClick(final View v) {
                    if(listaSuperAdmins.contains(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid())){
                        ...
                        builder.setTitle(v.getContext().getString(R.string.apagarpedidodefuncionalidade));

                        final TextView espaco2 = new TextView(v.getContext());
                        espaco2.setText(String.format("\n ✶ %s ✶ \n", funcionalidadesList.get(FunconalidadesViewHolder.getAdapterPosition()).getLogText()));
                        ...
                        builder.setPositiveButton(v.getResources().getString(R.string.apagar), new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                FirebaseDatabase.getInstance().getReference().child("logError").child(funcionalidadesList.get(FunconalidadesViewHolder.getAdapterPosition()).getUid()).removeValue();
                                funcionalidadesList.remove(FunconalidadesViewHolder.getAdapterPosition());
                                Snackbar.make(v,  v.getContext().getString(R.string.pedidodefuncionalidadeapagado), Snackbar.LENGTH_LONG)
    ...
    class FunconalidadesViewHolder extends EmptyRecyclerView.ViewHolder{
        final LinearLayout mLayout ;
        final TextView mLogText;
        final TextView mLoguserID;
        FunconalidadesViewHolder(View view){
            super(view);
