...
public class ChangeCreatorSalaPublicaAdapter extends EmptyRecyclerView.Adapter<ChangeCreatorSalaPublicaAdapter.ChangeCreatorViewHolder> {
    private final ArrayList<UserObject> userList;

    public ChangeCreatorSalaPublicaAdapter(ArrayList<UserObject> userList) {
        this.userList = userList;
    }

    @NonNull
    @Override
    public ChangeCreatorSalaPublicaAdapter.ChangeCreatorViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        return new ChangeCreatorSalaPublicaAdapter.ChangeCreatorViewHolder(layoutView);
    }

    @Override
    public void onBindViewHolder(@NonNull final ChangeCreatorSalaPublicaAdapter.ChangeCreatorViewHolder changeCreatorViewHolder, int i) {
            ...
            public void onClick(final View v) {
                ...
                builder3.setPositiveButton(v.getContext().getString(R.string.transferir), new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        FirebaseDatabase.getInstance().getReference().child("salasPublicas").child(userList.get(changeCreatorViewHolder.getAdapterPosition()).getSalaPrivadaID()).child("creator").setValue(userList.get(changeCreatorViewHolder.getAdapterPosition()).getUid()).addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                                FirebaseDatabase.getInstance().getReference().child("salasPublicas").child(userList.get(changeCreatorViewHolder.getAdapterPosition()).getSalaPrivadaID()).child("admins").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
