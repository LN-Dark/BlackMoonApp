...
public class UserListAdapter extends RecyclerView.Adapter<UserListAdapter.UserListViewHolder>  implements Filterable {
    ...
    public void onBindViewHolder(@NonNull final UserListViewHolder userListViewHolder, int i) {
        ...
        if(userList.get(userListViewHolder.getAdapterPosition()).getIsonline().equals("true")){
            ...
            public void onClick(final View v) {
                ...
                datainscri.setText(v.getResources().getString(R.string.utilizadorregistadodesde) + userList.get(userListViewHolder.getAdapterPosition()).getRegistoData() + "  -  " +userList.get(userListViewHolder.getAdapterPosition()).getRegistoHora());
                ...
                if (userList.get(userListViewHolder.getAdapterPosition()).getSociavel().equals("true")){
                    ...
                    builder3.setNeutralButton(v.getContext().getString(R.string.enviarpedido), new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            DatabaseReference mAppKeysRef = FirebaseDatabase.getInstance().getReference().child("appKeys");
                            mAppKeysRef.addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot578) {
                                    if(dataSnapshot578.exists()) {

                                        for (DataSnapshot childSnapshotfcm : dataSnapshot578.getChildren()) {
                                            if ("serverFCM".equals(Objects.requireNonNull(childSnapshotfcm.getKey()))) {
                                                FCMServer = (childSnapshotfcm.getValue().toString());
                                                break;
                                            }
                                        }
                                        final ArrayList<String> idsPedidosUser = new ArrayList<>();
                                        FirebaseDatabase.getInstance().getReference().child("user").child(userList.get(userListViewHolder.getAdapterPosition()).getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
                                            @Override
                                            public void onDataChange(@NonNull DataSnapshot dataSnapshot2323) {
                                                if(dataSnapshot2323.exists()){
                                                    if(dataSnapshot2323.child("pedidosComunidade").exists()){
                                                        for(DataSnapshot childpedidosSnapShot : dataSnapshot2323.child("pedidosComunidade").getChildren()){
                                                            idsPedidosUser.add(childpedidosSnapShot.getKey());
                                                        }
                                                        if(!idsPedidosUser.contains(FirebaseAuth.getInstance().getCurrentUser().getUid())){
                                                            FirebaseDatabase.getInstance().getReference().child("user").child(userList.get(userListViewHolder.getAdapterPosition()).getUid()).child("pedidosComunidade").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue("true").addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                @Override
                                                                public void onComplete(@NonNull Task<Void> task) {
                                                                    Snackbar.make(v.getRootView(), v.getResources().getString(R.string.pedidoenviado), Snackbar.LENGTH_LONG)
                                                                            .setAction("Action", null).show();
                                                                }
                                                            });
                                                        }else {
                                                            Snackbar.make(v.getRootView(), v.getResources().getString(R.string.jaenviasteumpedidoagiarda), Snackbar.LENGTH_LONG)
                                                                    .setAction("Action", null).show();
                                                        }
                                                    }else {
                                                        FirebaseDatabase.getInstance().getReference().child("user").child(userList.get(userListViewHolder.getAdapterPosition()).getUid()).child("pedidosComunidade").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue("true").addOnCompleteListener(new OnCompleteListener<Void>() {
    ...
    class UserListViewHolder extends EmptyRecyclerView.ViewHolder{
